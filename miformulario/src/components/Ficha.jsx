...
import catImage from '../assets/cat-4189697_640.png';
...
function Ficha({ animal }) {
  ...
  return (
    <Container sx={{ textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', mt: 4 }}>
      <Paper elevation={6} sx={{ p: 4, borderRadius: 3, textAlign: 'center', width: '100%', maxWidth: 500, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        ...
        <Box sx={{ mt: 2 }}>
          ...
          {animal === 'perro' && (
            <Button variant="contained" sx={{ backgroundColor: color, color: '#fff' }} onClick={() => setContadorResto(prev => Math.max(prev - 1, 0))}>
              {textoBotonPerder}
            </Button>
          )}
        </Box>
